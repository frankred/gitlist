...
class AdminController implements ControllerProviderInterface {
	...
	public function connect(Application $app) {
		...
		# Logout
		$route -> get('/admin/logout', function() use ($app, $session) {
			$session -> invalidate();
			return $app['twig'] -> render('admin_login.twig', array());
		}) -> bind('logout');

		# Login
		$route -> get('/admin/login', function() use ($app, $session) {
			if ($session -> has('admin')) {
				return $this -> adminpanel($app, $session);
			} else {
				return $app['twig'] -> render('admin_login.twig', array());
			}
		}) -> bind('login');
		...
		# Login (form submit)
		$route -> post('/admin/login', function(Request $request) use ($app, $session) {
			...
			if ($app['admin.name'] == $name && $app['admin.password'] == $password) {
				# log admin in
				$session -> set('admin', $app['admin.name']);
		...
		# Adminpanel
		$route -> get('/admin/admin', function() use ($app, $session) {
			return $this -> adminpanel($app, $session);
		}) -> bind('adminpanel');

		# AJAX: Adminpanel: Delete a repositories path out of the configuration
		$route -> get('/admin/admin/delete/{repospathBase64}', function($repospathBase64) use ($app, $session) {
			if ($session -> has('admin')) {
		...
		$route -> post('/admin/admin', function(Request $request) use ($app, $session) {
			if ($session -> has('admin')) {
				...
				return $app['twig'] -> render('admin.twig', array('repositorypaths' => $this -> repositoriesAddBase64Attribute($data['git']['repositories']), 'client' => trim($data['git']['client'], '"'), 'default_branch' => $data['git']['default_branch'], 'cache' => (empty($data['app']['cache']) || $data['app']['cache'] == 'false') ? false : true, 'debug' => (empty($data['app']['debug']) || $data['app']['debug'] == 'false') ? false : true, 'notification' => $notification, 'newpath' => $newpath));
			} else {
				return $app['twig'] -> render('admin_login.twig', array());
			}
		}) -> bind('savetoconfig');
	...
	private function adminpanel($app, $session) {
		if ($session -> has('admin')) {
			return $app['twig'] -> render('admin.twig', array('repositorypaths' => $this -> repositoriesAddBase64Attribute($app['git.repos']), 'client' => trim($app['git.client'], '"'), 'default_branch' => $app['git.default_branch'], 'cache' => (empty($app['twig.options']['cache']) || $app['twig.options']['cache'] == 'false') ? false : true, 'debug' => (empty($app['debug']) || $app['debug'] == 'false') ? false : true, 'admin' => $session -> get('admin')));
		} else {
			return $app['twig'] -> render('admin_login.twig', array());
		}
	}
